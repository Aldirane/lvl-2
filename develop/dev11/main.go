...
// Использовать go run calendar для запуска, вместо go run main.go
func main() {
	...
	localStorage := new(event.LocalStorage)
	repo := repository.EventRepository{}
	connApi := api.NewAPI(repo, localStorage)
	router := connApi.NewRouter()
	...
	go func() {
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("can't start server address %s\n", config.HTTPServerAddress)
		}
	}()
	...
	<-stop
	log.Println("received interrupt signal, closing server")
	timeout, cancel := context.WithTimeout(context.Background(), time.Duration(5*time.Second))
	defer cancel()
	if err := srv.Shutdown(timeout); err != nil {
		log.Fatalf("can't shutdown http server address %s, error %v", config.HTTPServerAddress, err)
	}
}
